...
describe('<CourseListRow />', () => {
  it('renders the correct content when isHeader true and textSecondCell null', () => {
    const courseListRow = shallow(<CourseListRow isHeader={true} textFirstCell="ok" />);
  ...
  it('renders the correct content when isHeader true and textSecondCell not null', () => {
    const courseListRow = shallow(<CourseListRow isHeader={true} textFirstCell="test" textSecondCell="ok" />);
  ...
  it('renders the correct content when isHeader is false', () => {
    const courseListRow = shallow(<CourseListRow isHeader={false} textFirstCell="test" />);
    expect(courseListRow.find('tr td')).toHaveLength(2);
  });
});
